...
<form action="{{route('prestasi.update', ['prestasi' => $prestasi->id])}}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <x-input type="text" id="title" name="title" label="Nama Prestasi"  :value="$prestasi->title" />
    <x-input type="date" id="tanggal" name="tanggal" label="Tanggal"  :value="$prestasi->tanggal" />
    <x-input type="text" id="lokasi" name="lokasi" label="Lokasi"  :value="$prestasi->lokasi" />
    ...
    <x-select :options="$kategori" name="kategori_id" id="kategori_id" label="Kategori"  :selected="$prestasi->kategori_id" />
    <div class="mb-2">
    ...
    <x-input id="image" name='image' type="file" label='Gamabr'  />
